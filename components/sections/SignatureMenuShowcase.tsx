...
// Curated for visual variety — one from each format.
...
export default function SignatureMenuShowcase() {
  ...
  return (
    <section className="relative overflow-hidden mx-auto max-w-7xl px-6 py-32 md:px-10">
      <div
        ref={glowRef}
        aria-hidden
        className="grain pointer-events-none absolute inset-0 opacity-50"
        style={{ background: "radial-gradient(50% 45% at 85% 10%, rgba(148,75,75,0.14), transparent 70%)" }}
      />
      <AnimatedSection variant="mask" className="relative mb-14 text-center">
        <p className="eyebrow mb-4" data-reveal>Our Signature</p>
        <AnimatedHeading
          as="h2"
          className="font-display text-3xl uppercase text-cream md:text-4xl"
          text="Worth Coming Back For"
        />
      </AnimatedSection>
      ...
      <div className="relative mt-14 text-center">
        <Link href="/menu">
          <SecondaryButton>See Full Menu</SecondaryButton>
        </Link>
      </div>
    </section>
  );
